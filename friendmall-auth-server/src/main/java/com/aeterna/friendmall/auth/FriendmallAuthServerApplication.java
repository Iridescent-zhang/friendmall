 ...
 *          3. 原先获取session是HttpServletRequest request.getSession(),现在变成了SessionRepositoryRequestWrapper.getSession()【装饰者模式】
 *                 后者是从SessionRepository去获取session的，而联系1)、1.知它正是在redis中找
 *【装饰者模式】，所有请求过来，将原生请求一包装，以后获取session都是调用的我的wrapper里规定的方法，我偷偷替它去从redis存取session
 *
