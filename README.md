...
友谊商城项目致力于打造一个完整的大型分布式架构电商平台，采用现阶段流行技术来实现，采用前后端分离编写。
基于[谷粒商城](https://www.bilibili.com/video/BV1np4y1C7Yf?from=search&seid=8989733132604162058)的学习项目。

## 项目介绍

友谊商城项目是一套电商项目，包括前台商城系统以及后台管理系统，基于 SpringCloud、SpringCloud Alibaba、MyBatis Plus实现。前台商城系统包括：用户登录、注册、商品搜索、商品详情、购物车、订单、秒杀活动等模块。后台管理系统包括：系统管理、商品系统、优惠营销、库存系统、订单系统、用户系统、内容管理等七大模块。
...
|  renren-generator  | 人人开源项目的CRUD代码生成器 |   https://gitee.com/renrenio/renren-generator   |
|   Elasticsearch    |         搜索引擎         |    https://github.com/elastic/elasticsearch     |
|      RabbitMQ      |         消息队列         |            https://www.rabbitmq.com             |
|   Springsession    |        分布式Session        |    https://projects.spring.io/spring-session    |
...
注意：以上的除了jdk都是采用docker方式进行安装，详细安装步骤请上网查询。
...
```
192.168.56.10 friendmall.com
192.168.56.10 search.friendmall.com
192.168.56.10 item.friendmall.com
192.168.56.10 auth.friendmall.com
192.168.56.10 cart.friendmall.com
192.168.56.10 order.friendmall.com
192.168.56.10 member.friendmall.com
192.168.56.10 seckill.friendmall.com
以上ip换成自己Linux的ip地址
```
...
2、在friendmall.conf中添加如下配置
server {
	# 监听以下域名地址的80端口
    listen       80;
    server_name  friendmall.com  *.friendmall.com hjl.mynatapp.cc;
    ...
    #支付异步回调的一个配置
    location /payed/ {
        proxy_set_header Host order.friendmall.com;        #不让请求头丢失
        proxy_pass http://friendmall;
    }
...
- 克隆整个后端项目 `friendmall` ，并导入 IDEA 中完成编译
